    ...
    public class ExcelWorksheet : XmlHelper, IEqualityComparer<ExcelWorksheet>, IDisposable
    {
        ...
        private void DrawingsDeleteRow(int rowFrom, int rows)
        {
            ...
            foreach (ExcelDrawing d in Drawings)
            {
                ...
                for (int i = 0; i <= Drawings.Max(p => p.To.Row); i++)
                {
                    if (i >= d.From.Row && i <= d.To.Row)
                    {
                        kolekcja.Add(indeksGrafiki);
                    }
                    else
                    {
                        kolekcja.Add(null);
                ...
                kolekcjeGrafikiList.Add(kolekcja);
                indeksGrafiki++;
            }

            foreach (var kolGr in kolekcjeGrafikiList)
            {
                if (rowFrom > kolGr.Count)
                {
                    continue;
                }
                if (kolGr.Count < rows + rowFrom)
                {
                    kolGr.RemoveRange(rowFrom - 1, kolGr.Count - rowFrom + 1);
                }
                else
                {
                    kolGr.RemoveRange(rowFrom - 1, rows);
                }
            }
            ...
            for (int i = 0; i < Drawings.Count; i++)
            {
                ...
                if (d.EditAs != eEditAs.Absolute)
                {
                    ...
                    if (nrWiersza < 0)
                    {
                        obrazkiDoUsunieciaList.Add(indeksGrafiki);
                        indeksGrafiki++;
                        continue;
                    }
            ...
            foreach (var ou in obrazkiDoUsunieciaList.OrderByDescending(p => p))
            {
                Drawings.Remove(ou);
            }
        }
        ...
        public void DeleteColumn(int columnFrom, int columns)
        {
            ...
            lock (this)
            {
                ...
                DrawingsColumnsDelete(columnFrom, columns);
        ...
        private void DrawingsColumnsDelete(int columnFrom, int columns)
        {
            var kolekcjeGrafikiList = new List<List<int?>>();
            var indeksGrafiki = 0;
            foreach (ExcelDrawing d in Drawings)
            {
                var kolekcja = new List<int?>();
                for (int i = 0; i <= Drawings.Max(p => p.To.Column); i++)
                {
                    if (i >= d.From.Column && i <= d.To.Column)
                    {
                        kolekcja.Add(indeksGrafiki);
                    }
                    else
            ...
            foreach (var kolGr in kolekcjeGrafikiList)
            {
                if (columnFrom > kolGr.Count)
                {
                    continue;
                }
                if (kolGr.Count < columns + columnFrom)
                {
                    kolGr.RemoveRange(columnFrom - 1, kolGr.Count - columnFrom + 1);
                }
                else
                {
                    kolGr.RemoveRange(columnFrom - 1, columns);
                }
            }
            ...
            foreach (ExcelDrawing d in Drawings)
            {
                if (d.EditAs != eEditAs.Absolute)
                {
                    var nrKolumny = kolekcjeGrafikiList[indeksGrafiki].FindIndex(p => p == indeksGrafiki);
                    if (nrKolumny < 0)
                    {
                        obrazkiDoUsunieciaList.Add(indeksGrafiki);
                        indeksGrafiki++;
                    ...
                    var nrKolumnyDo = kolekcjeGrafikiList[indeksGrafiki].FindLastIndex(p => p == indeksGrafiki);
                    if (d.EditAs == eEditAs.OneCell)
                    {
                        if (nrKolumnyDo - nrKolumny < d.To.Column - d.From.Column)
                        {
                            d.To.Column = nrKolumny + d.To.Column - d.From.Column;
                            d.From.Column = nrKolumny;
                            indeksGrafiki++;
                            continue;
                        }
                    }
                    d.From.Column = nrKolumny;
                    d.To.Column = nrKolumnyDo;
                }
                indeksGrafiki++;
            }
